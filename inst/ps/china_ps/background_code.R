...
#rdplot2 and functions-code from rdplot package
...
#Veränderungen am rdplot package:
#Veränderung der Reihenfolge --> geom_line erst später, um die lines im Vordergrund zu haben
#kosmetische Veränderungen, z.B. Farbe bei ci, Breite der lines, geom_ribbon, hinzufügen von theme + scale_x_continous, print


rdplot2 = function(y, x, c=0, p=4, nbins = NULL, binselect = "esmv", scale = NULL, 
                   ...
                   col.dots = NULL, col.lines = NULL, blank_vert_grid_lines = NULL, scale_x_breaks = NULL, scale_y_breaks = NULL) {
  ...
  if (hide=="FALSE") {
    ...
    temp_plot <- ggplot() + theme_bw() +
      geom_point(data=data_bins, size = 1.3, aes(x=rdplot_mean_bin, y=rdplot_mean_y), col=col.dots, na.rm=TRUE)  #size
    
    
    if (flag_no_ci==FALSE)
      temp_plot <- temp_plot +
      geom_errorbar(data=data_bins, size = 0.5, col = "grey75", aes(x=rdplot_mean_bin, ymin=rdplot_cil_bin, ymax=rdplot_cir_bin), linetype = 1) #col, size
    if (shade==TRUE){
      temp_plot <- temp_plot +
        geom_ribbon(data=data_bins, aes(x=rdplot_mean_bin, ymin=rdplot_cil_bin-10, ymax=rdplot_cir_bin+10), fill= "grey85") #fill und -10/+10
    }
    if (blank_vert_grid_lines==TRUE){
      temp_plot <- temp_plot +
    ...
    temp_plot <- temp_plot + labs(x = x.label, y = y.label) + ggtitle(title)+
      ...
      geom_line( data=data_poly, aes(x=x_plot_l, y=y_hat_l), col=col.lines, na.rm=TRUE) + #geom_line wurde hier hin verschoben
      geom_line( data=data_poly, aes(x=x_plot_r, y=y_hat_r), col=col.lines, na.rm=TRUE) +#geom_line wurde hier hin verschoben
...
#function needed for rdrobust output using modelsummary
...
glance.rdrobust = function(object, ...){
  ret <- data.frame(Observations = format(object$N_h[1] + object$N_h[2], big.mark=",", scientific = FALSE),
                    Bandwidth = format(round(object$bws[1],0), big.mark=",", scientific = FALSE),
                    Kernel = substr(object$kernel, 1, 3))
  ret
}
)
...
library(RTutor)
copy.into.env(my_funs, globalenv())


#copy with division left and right
#glance.rdrobust = function(object, ...){
#  ret <- data.frame(obs_total = format(object$N_h[1] + object$N_h[2], big.mark=",", scientific = FALSE),
#                    obs_left = format(object$N_h[1], big.mark=",", scientific = FALSE),
#                    obs_right = format(object$N_h[2], big.mark=",", scientific = FALSE),
#                    bandwidth_left = format(round(object$bws[1],0), big.mark=",", scientific = FALSE),
#                    bandwidth_right = format(round(object$bws[3],0), big.mark=",", scientific = FALSE),
...
#avoiding conflicts with the dplyr select function
select <- dplyr::select
